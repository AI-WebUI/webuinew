...
$(function(){
    //增加按钮
    $(".demo_content").prepend('<span class="btn-clipboard">查看源码</span>');

    //默认隐藏代码区域
    $(".highlight").hide();
    $(".demo_content").find(".btn-clipboard:first").click(function(){
        if($(this).next().next(".highlight").is(":hidden")){
            $(this).next().next(".highlight").slideDown();
            $(this).text("隐藏源码");
        }else{
            $(this).next().next(".highlight").slideUp();
            $(this).text("显示源码");
        }
    });

    $(".highlight").each(function(){
        $(this).html('<pre class="code" data-language="html">'+up($(this).prev().html())+'</pre>');
    })

    //UBB转换器
    function up(str){
        //移除源码代码中无效的代码
        str = str.replace('style="position:absolute;"',"");
